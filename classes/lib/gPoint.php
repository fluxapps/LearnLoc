...
//
	function UTMLetterDesignator() {
		if((84 >= $this->lat) && ($this->lat >= 72))
			...
			if((0 > $this->lat) && ($this->lat >= -8)) {
				$LetterDesignator = 'M';
			}
			else {
				if((-8 > $this->lat) && ($this->lat >= -16)) {
					$LetterDesignator = 'L';
				}
				else {
					if((-16 > $this->lat) && ($this->lat >= -24)) {
						$LetterDesignator = 'K';
					}
					else {
						if((-24 > $this->lat) && ($this->lat >= -32)) {
							$LetterDesignator = 'J';
						}
						else {
							if((-32 > $this->lat) && ($this->lat >= -40)) {
								$LetterDesignator = 'H';
							}
							else {
								if((-40 > $this->lat) && ($this->lat >= -48)) {
									$LetterDesignator = 'G';
								}
								else {
									if((-48 > $this->lat) && ($this->lat >= -56)) {
										$LetterDesignator = 'F';
									}
									else {
										if((-56 > $this->lat) && ($this->lat >= -64)) {
											$LetterDesignator = 'E';
										}
										else {
											if((-64 > $this->lat) && ($this->lat >= -72)) {
												$LetterDesignator = 'D';
											}
											else {
												if((-72 > $this->lat) && ($this->lat >= -80)) {
													$LetterDesignator = 'C';
												}
												else {
													$LetterDesignator = 'Z';
...
//		$y -= 10000000.0;	// Uncomment line to make LOCAL coordinates return southern hemesphere Lat/Long
		...
		$tlat      = $phi1Rad - ($N1 * tan($phi1Rad) / $R1) * ($D * $D / 2 - (5 + 3 * $T1 + 10 * $C1 - 4 * $C1 * $C1 - 9 * $eccPrimeSquared) * $D * $D * $D * $D / 24 + (61 + 90 * $T1 + 298 * $C1 + 45 * $T1 * $T1 - 252 * $eccPrimeSquared - 3 * $C1 * $C1) * $D * $D * $D * $D * $D * $D / 720); // fixed in 1.1
...
//
	function configLambertProjection($falseEasting, $falseNorthing, $longOfOrigin, $latOfOrigin, $firstStdParallel, $secondStdParallel) {
